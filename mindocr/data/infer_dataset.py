...
class InferDataset(BaseDataset):
    ...
    def __init__(self,
                 ...
                 **kwargs):
        img_dir = os.path.join(dataset_root, data_dir)   # TODO: whether need to set dataset_root as input args?
            ...
            for k in output_columns:
                if k in _data:
                    ...
                    raise ValueError(f"Key '{k}' does not exist in data (available keys: {_data.keys()}). "
                                     "Please check the name or the completeness of transformation pipeline.")
    ...
    def load_data_list(self,
        ...
        # idx = 0 # TODO: whether support mulitple image folder as input, sample_ratio can be a list
        if shuffle:
